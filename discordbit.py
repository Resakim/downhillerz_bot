...
async def on_message(message):
    ...
    if message.content.startswith("!help"):
        ...
        embed.add_field(name=1️⃣신고 대상자가 명확해야 합니다, value= 채팅, 인게임 닉네임은 관리자가 식별할 수 있어야 합니다., inline=False)
        embed.add_field(name=2️⃣신고 제보 작성요령, value=제보내용은 육하원칙에 따라 핵심을 쉽게 파악할 수 있도록 작성해야됩니다.\n 게임내 부정행위, 명예훼손등 왜 그런 일이 발생하게 되었는지\n 전 과정을 소상히 작성 부탁드립니다., inline=False)
        embed.add_field(name=3️⃣증거자료, value=이미지, 동영상, 링크 등으로 첨부합니다.\n 이는 필수적이며 미제출시 사실관계를 확인할 수 없습니다.\n (거짓진술 또는 본인에게 유리하게 편집, 왜곡하여 제출 시 신고자 처벌), inline=False)
        embed.add_field(name=❌아래에 해당하는 경우 신고가 처리되지 않습니다, value= , inline=False)
        embed.add_field(name=🔸사건 종료 이후 1시간 이상 경과된 사건, value= , inline=False)
        embed.add_field(name=🔸신고 대상자에게 거부의사를 명확히 알리지 않았을 경우, value= , inline=False)
        embed.add_field(name=🔸절차에 의하지 않고 자력으로 권리를 실현, 구제하는 경우, value=질서 위반행위에 대하여 처벌받을 수 있음, inline=False)
        embed.add_field(name=🔸신고 대상자가 사과를 했음에도 받아들여지지 않은 경우, value=사과의 진정성, 사건의 종결 상황에 따라 다를 수 있음, inline=False)
        embed.add_field(name=🔸패시브 모드(Passive)를 비활성화 한 차량충돌, value=주행로 외 주차된 차량을 고의로 파손한 행위는 제외, inline=False)
